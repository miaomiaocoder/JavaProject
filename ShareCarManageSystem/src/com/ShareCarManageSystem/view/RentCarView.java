...
public class RentCarView extends javax.swing.JFrame {
    ...
    //数据标题
    private String headTitle[] = new String[]{"租车编号", "租车时间", "租车用户", "车辆ID", "租车费用"};
    private DefaultTableModel dtm = null;

    public Object[][] makeTable() {
        List<RentCar> list = Database.queryRentCarInfo();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("租车编号");

        jLabel2.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel2.setText("租车时间");

        jLabel3.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel3.setText("租车用户");

        jLabel4.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel4.setText("车辆ID");

        jLabel5.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel5.setText("租车费用");
    ...
    private void jbtn_addActionPerformed(java.awt.event.ActionEvent evt) {                                         
        // 添加车辆
        RentCar car = new RentCar(0, jtf_time.getText(), jtf_username.getText(), Integer.parseInt(jtf_carId.getText()),Float.parseFloat(jtf_status.getText()));
        boolean result = Database.addRentCarInfo(car);
        if (result) {
            JOptionPane.showMessageDialog(null, "添加租车成功!", "提示", JOptionPane.PLAIN_MESSAGE);
            //刷新列表
            ...
            JOptionPane.showMessageDialog(null, "添加租车失败!", "提示", JOptionPane.ERROR_MESSAGE);
        }
    }                                        

    private void jbtn_modifyActionPerformed(java.awt.event.ActionEvent evt) {                                            
        //修改车辆
        RentCar car = new RentCar(Integer.parseInt(jtf_id.getText()), jtf_time.getText(), jtf_username.getText(), Integer.parseInt(jtf_carId.getText()),Float.parseFloat(jtf_status.getText()));
        boolean result = Database.modifyRentCarInfo(car);
        if (result) {
            JOptionPane.showMessageDialog(null, "修改租车成功!", "提示", JOptionPane.PLAIN_MESSAGE);
            //刷新列表
            jbtn_refresh.doClick();
        } else {
            JOptionPane.showMessageDialog(null, "修改租车失败!", "提示", JOptionPane.ERROR_MESSAGE);
        }
    }                                           

    private void jbtn_deleteActionPerformed(java.awt.event.ActionEvent evt) {                                            
        //删除车辆
        boolean result = Database.deleteRentCarInfo(Integer.parseInt(jtf_id.getText()));
        if (result) {
            JOptionPane.showMessageDialog(null, "删除租车成功!", "提示", JOptionPane.PLAIN_MESSAGE);
            //刷新列表
            jbtn_refresh.doClick();
            //清空
            jtf_id.setText("");
            ...
            JOptionPane.showMessageDialog(null, "删除租车失败!", "提示", JOptionPane.ERROR_MESSAGE);
        }
    }                                           
